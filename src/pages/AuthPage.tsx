...
import { Mail, Lock, User, Phone, ArrowLeft, Scissors } from "lucide-react";
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (user) {
          await supabase.from("profiles").update({ phone_number: phone, full_name: fullName }).eq("id", user.id);
        }
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex items-center gap-3 p-4 border-b border-border">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-display font-bold text-foreground">The King's Cut</span>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center p-6 max-w-lg mx-auto w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-2">
            {isLogin ? "Inicia sesión para agendar tu cita" : "Regístrate para reservar tu primera cita"}
          </p>
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4">
          ...
          <Button variant="gold" className="w-full h-12 rounded-xl text-base" disabled={loading}>
